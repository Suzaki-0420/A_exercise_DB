...
public class EmployeeRepositoryTests
{
    ...
    public async Task FindAllWithoutAccountAsync_WhenEmployeesExist_ShouldReturnEmployeeList()
    {
        ...
        var yamada = employees.SingleOrDefault(
            e => e.EmployeeUuid ==
                Guid.Parse("11111111-1111-1111-1111-111111111111"));

        Assert.IsNotNull(yamada);
        Assert.AreEqual("山田 太郎", yamada.Name);
        Assert.AreEqual("ヤマダ タロウ", yamada.Kana);
        Assert.IsNotNull(yamada.Department);
        Assert.AreEqual(
            Guid.Parse("e480fa43-f51c-4738-93dc-fb4fe0ecea42"),
